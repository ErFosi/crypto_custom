...
func signingParamsForPublicKey(pub any, requestedSigAlgo SignatureAlgorithm) (hashFunc crypto.Hash, sigAlgo pkix.AlgorithmIdentifier, err error) {
	...
	default:
		...
		/*} else {
			print(pub)
			err = errors.New("x509: only RSA, ECDSA, Ed25519 and Dilithium2 keys supported")
		}*/
...
func esDilithium(clavePublica interface{}) bool {
	claveBytes, ok := clavePublica.([]byte)
	if !ok {
		fmt.Println("La clave pública no es del tipo esperado ([]byte).")
		return false
	}

	// Asumiendo que la clave pública de Dilithium comienza con "Dilithium"
	return string(claveBytes[:9]) == "Dilithium"
}
...
func CreateCertificateRequest(rand io.Reader, template *CertificateRequest, priv any) (csr []byte, err error) {
	...
	if hashFunc != 0 {
		h := sha3.New256() // Crea un nuevo hash SHA3-256
		h.Write(signed)
		signed = h.Sum(nil)
	}
